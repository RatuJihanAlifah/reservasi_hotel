...
if (isset($_POST['get_bookings'])) 
{
  ...
  AND (bo.order_id LIKE ? OR bd.email LIKE ? OR bd.full_name LIKE ?)
  ...
  if($total_rows==0){
    $output = json_encode(["tabel_data"=>"<b>No Data Found!</b>","pagination"=>'']);
    echo $output;
    exit;
  }
  ...
  while($data = mysqli_fetch_assoc($limit_res)) {
      ...
      <tr>
          ...
          <td>
              <button type='button' onclick='download({$data['booking_id']})' class='btn btn-outline-primary btn-sm fw-bold shadow-none'>
  ...
  $output = json_encode(["tabel_data"=>$tabel_data,"pagination"=>$pagination]);
  echo $output;
}
