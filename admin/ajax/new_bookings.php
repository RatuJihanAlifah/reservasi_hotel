...
if (isset($_POST['get_bookings'])) 
{
  ...
  WHERE (bo.order_id LIKE ? OR bd.email LIKE ? OR bd.full_name LIKE ?)
  AND (bo.booking_status = ? AND bo.arrival = ?) ORDER BY bo.booking_id ASC";

  $res = select($query,
    ["%$frm_data[search]%","%$frm_data[search]%","%$frm_data[search]%","pending",0],'sssss');
  $i = 1;
  $tabel_data = "";
  ...
  while($data = mysqli_fetch_assoc($res)) {
      ...
      $tabel_data .="
        <tr>
            ...
            <td>
              <button type='button' onclick='assign_room($data[booking_id])' class='btn btn-info text-white btn-sm fw-bold shadow-none' data-bs-toggle='modal' data-bs-target='#assign-room'>
